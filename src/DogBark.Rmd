...
abline(v=c(startTime,endTime), col="red")
quickSampleStartTime = 1.15 + 0.05 * c(0:4)
...
#indexPerWave = sample.rate / arfFOI 

for (st in quickSampleStartTime){
	en = st + (3 * max(secPerFqInterval))
	x = seq(st,en,1)/sample.rate
	y = b1[st:en]
	plot(x=x, y=y, col="darkblue", las=1, xlab=xlabTime, ylab=ylab)
	title(paste("dog bark sampled at", st, "seconds"))
	abline(v=c(st,en), col="orange")
	abline(v=(st+max(secPerFqInterval)*c(1,2))/sample.rate, col="red", lty=2)
	for (i in 1:length(indexPerWave)){
		segments(x0=seq(st, en, indexPerWave[i])/sample.rate, 
						 y0=par("usr")[4], y1=par("usr")[4]*.7, 
						 col="gray", lty=2)
		# fq = freqName[i]
		# x = st:en
		# lines(x, y=sin(freq.of.interest[i] * 2 * pi * x), col=i+1)
	}
}
```
