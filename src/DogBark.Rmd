...
```{r}
plot(getTimes(numB1), numB1, type="l", col="darkblue", las=1, xlim=c(1.10, 1.25)) #, xlim=c(50000,54000)
maxAmp = max(numB1)
...
points(allEnv[,2], allEnv[,1], col="red", pch=16)
...
nrow=length(numB1)
ncol=length(peak)
# bt = matrix(data=numB1 < threshold, byrow = F, nrow=nrow, ncol=ncol)
bt = matrix(data=shape3 < threshold, byrow = F, nrow=nrow, ncol=ncol)
...
isBefore = bt & bp
#startInd = colSums(isBefore)
startInd = apply(isBefore, 2, function(x){max(which(x))})
abline(v=startInd, col="purple")
isAfter = bt & !bp
endInd = apply(isAfter, 2, function(x){min(which(x))})
abline(v=endInd, col="blue")

drawAt = seq(.1 * maxAmp, .9 * maxAmp, length.out = 5)
y0 = drawAt[ (0:(nrow(allEnv)-1) %% length(drawAt)) + 1]
segments(x0=allEnv$start, x1=allEnv$end, y0=y0, col="red")
abline(v=allEnv$start, col="red")

abline(h=minpeakheight, col="green")
