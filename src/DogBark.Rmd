...
```{r}
getBestFreqs = function(ft, freqs, lowSpan, small){
...
freqName = as.character(freq.of.interest / (2 * pi))
...
for (i in 1:length(freq.of.interest)){
	...
	rebuilt[,i] = sin(freq.of.interest[i] * x) * amplitudeFactor
}
```
...
getFreqOfInterest = function(ft, freqs,
														 small = quantile(abs(ft)[getMaxima(abs(ft))], .75), 
														 span = seq(0.01, .2, length.out = 40)){
	...
	for (i in 1:n.span){
		tryCatch(expr = {
			# suppressWarnings(expr={ft.lo = loess(abs(ft) ~ freqs, span=span[i])})
			# smoother = predict(ft.lo, freqs)
			# maxes = getMaxima(smoother)
			# numMaxes[i] = sum(smoother[maxes] > small)
			numMaxes[i] = suppressWarnings(expr={
				length(getBestFreqs(ft, freqs, lowSpan=span[i], small=small))
				})
		}, finally = {
	...
	#print(lowSpan)
	freq.of.interest = getBestFreqs(ft, freqs, lowSpan, small=small)
	return(freq.of.interest)
}
...
# image(x=midTime, y=as.numeric(colnames(plotMatrix)), z=plotMatrix, las=1, col=heat.colors(length(plotMatrix)))
```
