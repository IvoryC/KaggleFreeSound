...
plotSmoothMaxes(x=freq, y=abs(arf.ft), 
								dotsAt = arfMaxes, 
								thresh=quantile(abs(ft)[getMaxima(abs(ft))], .95))
```
...
getFreqOfInterest <- function(ft, freqs, small=quantile(abs(ft)[getMaxima(abs(ft))], .95)){
	inds = findpeaks(abs(ft), minpeakheight = small)
	freq.of.interest = freqs[inds[,2]]
}
